...
export default function Step18ProgressChart({ onNext, onBack, step, totalSteps, onSkip }: StepProps) {
  ...
  // Use the full innerWidth since OnboardingLayout already applies padding
  const chartWidth = innerWidth;
  const isMaintain = data.goal === 'maintain';
  const isLose = data.goal === 'lose';
  const startWeight = data.weightKg;
  const endWeight = isMaintain ? data.weightKg : data.targetWeightKg;
  const isImperial = data.unitSystem === 'imperial';

  const toDisplay = (kg: number) =>
    isImperial ? Math.round(kg * 2.20462) : Math.round(kg);
  const unit = isImperial ? 'lb' : 'kg';

  // Y positions depend on goal direction
  const xs = MONTHS.map((_, i) => (i / (MONTHS.length - 1)) * chartWidth);

  let ys: number[];
  if (isMaintain) {
    // Flat line with slight variation for visual interest
    const baseline = CHART_HEIGHT * 0.5;
    ys = [baseline, baseline - 6, baseline + 4, baseline];
  } else {
    const yStart = isLose ? CHART_HEIGHT * 0.15 : CHART_HEIGHT * 0.85;
    const yEnd = isLose ? CHART_HEIGHT * 0.85 : CHART_HEIGHT * 0.15;
    ys = MONTHS.map((_, i) => yStart + (yEnd - yStart) * (i / (MONTHS.length - 1)));
  }
  ...
  const goalDescription = isMaintain
    ? `Mantener tu peso en ${toDisplay(startWeight)} ${unit}`
    : isLose
      ? `Bajar de ${toDisplay(startWeight)} ${unit} a ${toDisplay(endWeight)} ${unit}`
      : `Subir de ${toDisplay(startWeight)} ${unit} a ${toDisplay(endWeight)} ${unit}`;
  ...
  return (
    ...
    >
      ...
      <Animated.View
        ...
        accessibilityLabel={`Grafico de progreso: ${goalDescription}`}
      >
        {/* Weight labels */}
        ...
        <Text style={styles.caption}>
          {isMaintain
            ? 'Mantener tu peso requiere constancia. Te ayudamos a lograrlo dia a dia.'
            : 'Segun tu objetivo y ritmo, podrias alcanzar tu peso ideal en 3 a 5 meses.'
          }
        </Text>
      </Animated.View>
    </OnboardingLayout>
